...
export async function getDexOrderBook(opts: {
  ...
  // Asks: amounts are in from_symbol (e.g. USDT with 6 decimals)
  // Bids: amounts are in to_symbol (e.g. BUILD with 9 decimals)
...
export async function getDexTradingStats(fromSymbol: string, toSymbol: string): Promise<DexTradingStats> {
  ...
  // Volumes are in from_symbol nano units
  const volFactor = 10 ** Number(data.from_decimals ?? 9);
