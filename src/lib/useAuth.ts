...
export function useAuth(baseCfg: PublicApiConfig): {
  ...
  // on initial mount (null → null) or transient TonConnect flickers.
  ...
  // Step 4: Listen for explicit disconnect via TonConnectUI status change.
  // This is more reliable than watching `wallet` which can flicker to null
  // during TonConnect bridge reconnections or internal state transitions.
  useEffect(() => {
